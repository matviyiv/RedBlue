...
environment running for the first time. One-time setup is `./ai-scripts/init.sh`;
...
flowchart TD
    ...
    choose{"How to run?"}
    choose -->|Interactive| cli["./ai-scripts/start-cli.sh"]
    choose -->|Headless / CI| hl["./ai-scripts/run-headless.sh \"<prompt>\""]
...
| 3 | **Run init** | `./ai-scripts/init.sh` | Verifies tools, resolves auth, marks scripts executable, seeds `.env.example`, lists the configured blue-zone folders, and builds the image. |
...
| 6 | **Run a session** | `./ai-scripts/start-cli.sh` or `./ai-scripts/run-headless.sh "…"` | Each run prepares + validates the blue zone, mounts it, and syncs changes back on exit. |
...
*and* headless runs reuse it. Wipe it with `./ai-scripts/start-cli.sh --clear`.
