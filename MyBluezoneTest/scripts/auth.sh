...
#   1. oauth-token      CLAUDE_CODE_OAUTH_TOKEN (sk-ant-oat...), a long-lived
#                       token created with `claude setup-token` from a
#                       Claude Pro/Max subscription
#   2. persisted-login  OAuth credentials saved in the claude-home Docker
#                       volume by a previous interactive /login session
#   3. none             nothing found — interactive sessions can still start
#                       and log in via /login; headless runs must abort
#
# Source this file, then call resolve_auth. It sets:
#   AUTH_MODE       one of: oauth-token | persisted-login | none
